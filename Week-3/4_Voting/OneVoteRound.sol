...
contract oneVotingRound {
    ...
    function registerCandidate() external checkVotingStarted checkVotingStopped("You are late. Voting is completed.") {
    ...
    function startVoting() external isAdmin("You are not authorised to start voting") checkVotingStarted checkVotingStopped("Voting is completed.") {
        // checking if min 1 candidate is registered
        require(totalRegisteredCandidates > 1, "No candidates registered");
        votingInProgress = true;
    }

    function stopVoting() external isAdmin("You are not authorised to stop voting") checkVotingStopped("Voting already stopped") {
    ...
    function vote(address _voteTo) external checkVotingStopped("Voting has been stopped") {
        ...
        // vote
        uint _totalVotes = CandidatesRecord[_voteTo].votes + 1;
        CandidatesRecord[_voteTo].votes = _totalVotes;

        if(_totalVotes > leadingCandidateVotes){
            leadingCandidate = _voteTo;
            leadingCandidateVotes = _totalVotes;
        }
    }
}